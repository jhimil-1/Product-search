...
def create_session():
    try:
        ...
        vectorstore.create_collection_if_not_exists(collection_name)
...
def upload_products(session_id):
    ...
    sessions_col.update_one({"_id": session_id}, {"$push": {"products": {"$each": products}}})

    return jsonify({"status": "ok", "saved_images": [f"/uploads/{f}" for f in saved_files]})
...
def query_text():
    ...
    return jsonify({"answer": f"Found {len(images)} similar products:", "images": images})
